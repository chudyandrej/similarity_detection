...
class Triplet(ComputingModel):
    OUTPUT_ROOT = f"{ComputingModel.OUTPUT_ROOT}/triplet"
    ...
    def train_model(self):
        ...
        ev = AuthorityEvaluator(username='andrej', neighbors=20, results_file=self.output_path)

        train_profiles, valid_profiles = ev.split_profiles(ev.s3_profiles, train_size=0.8)

        [anchors, positives, negatives], targets = next(triplet_generator(profiles=valid_profiles, batch_size=20000,
                                                                          preprocess_profile=self.preprocess_profile))

        time_callback = cc.TimeHistory()
        hist = model.fit_generator(generator=triplet_generator(train_profiles, 128, self.preprocess_profile),
                                   steps_per_epoch=1000,
                                   epochs=200,
                                   validation_data=([anchors, positives, negatives], targets),
                                   callbacks=[
