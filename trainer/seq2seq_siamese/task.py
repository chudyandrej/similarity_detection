...
import trainer.seq2seq_siamese.model as model
...
def main(data_file, job_dir):
    ...
    input_texts, target_texts, types = model.load_data(data_file[0])

    losses = {"dense": "categorical_crossentropy", "dotLayer": "binary_crossentropy"}
    loss_weights = {"dense": 1.0, "dotLayer": 0.8}
    model_seq2seq = model.create_model()
    model_seq2seq.compile(optimizer='rmsprop', loss=losses, loss_weights=loss_weights)

    model_seq2seq.fit_generator(model.generate_random_fit(input_texts, target_texts, types),
