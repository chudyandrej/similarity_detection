...
def create_model():
    ...
    decoder_inputs_1 = Input(shape=(None, TOKEN_COUNT), name="decoder_Input_1")
    decoder_inputs_2 = Input(shape=(None, TOKEN_COUNT), name="decoder_Input_2")
    ...
    decoder_outputs1, _, _ = decoder_lstm(decoder_inputs_1, initial_state=encoder_states1)
    decoder_outputs2, _, _ = decoder_lstm(decoder_inputs_2, initial_state=encoder_states2)
    ...
    model = Model([encoder_inputs_1, decoder_inputs_1, encoder_inputs_2, decoder_inputs_2],
                  [decoder_outputs1, decoder_outputs2, output])
    return model


def generate_random_fit(input_texts, target_texts, types):
    ...
    column_index = make_index_by_firs_value(zip(types, zip(input_texts, target_texts)))
    ...
    while True:
        ...
        encoder_input_data1, decoder_input_data1, decoder_target_data1 = convert_to_vec(batch_data1)
        encoder_input_data2, decoder_input_data2, decoder_target_data2 = convert_to_vec(batch_data2)

        distances = np.array(list(map(lambda x: int(x), match)))

        yield [encoder_input_data1, decoder_input_data1, encoder_input_data2, decoder_input_data2], \
              [decoder_target_data1, decoder_target_data2, distances]
...
def convert_to_vec(batch_data):
    ...
    for i, (input_text, target_text) in enumerate(batch_data):
